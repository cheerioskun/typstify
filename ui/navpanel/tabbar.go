...
func NewTabbar(vm view.ViewManager, options *TabbarOptions) *Tabbar {
	...
	//So we can use mouse wheel to scroll horizontally, without needing to press Shift
	tb.list.ScrollAnyAxis = true
	...
	dims := layout.Background{}.Layout(gtx,
		func(gtx C) D {
			return layout.Inset{
				...
				return tab.layoutBackground(gtx, th)
			})
		},
		func(gtx C) D {
...
func (tab *Tab) layoutBackground(gtx C, th *theme.Theme) D {
	...
	if tab.hovering {
		rect := clip.Rect{Max: image.Point{X: gtx.Constraints.Min.X, Y: gtx.Constraints.Min.Y}}
	...
	if tab.isSelected {
		...
		var path clip.Path
		path.Begin(gtx.Ops)
		path.Move(f32.Pt(0, float32(gtx.Constraints.Min.Y-width/2)))
		path.Line(f32.Pt(float32(gtx.Constraints.Min.X), 0))

		paint.FillShape(gtx.Ops, th.ContrastBg,
			clip.Stroke{
				Path:  path.End(),
				Width: float32(width),
			}.Op(),
